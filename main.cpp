...
#if defined(NDEBUG)
# error "TAcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TAcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TAcoin Signed Message:\n";
...
{
    // TAcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // TAcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // TAcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // TAcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TAcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TAcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The TA's congregated for the first time on July 11th, 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
       ...
       if (true && block.GetHash() != hashGenesisBlock)
        {
            printf("Searching for gensis block...\n");
            //Team This above statement will figure out a valid hash and Nonce if you're
            // creating a different genesis block
            uint256 hashTarget = CBigNum().SetCompact(block.nBits).getunit256();
            unit256 thash;
            ...
            loop
            {
                ...
                {
                    printf("nonce %08X: hash = %s (target = %s)\n", block.nNonce, thash.ToString().c_str(), hashTarget.ToString().c_$)
                }
                ++block.nNonce;
                if (block.nNonce == 0)
                {
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TAcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TAcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TAcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TAcoinMiner(CWallet *pwallet)
{
    printf("TAcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("TAcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TAcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TAcoinMiner, pwallet));
}
